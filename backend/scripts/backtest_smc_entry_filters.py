...
#
# Re-run on the FULL HOSE/HNX universe (220 tickers) -- ranking held, and the
# 0.33-vs-0.5 gap all but vanished at the larger sample, vindicating the
# doctrine-over-fitted choice:
#   POI-third-ALL+RR3  n= 296  mean_r=+3.54  median=+0.75  ci=[0.275, 0.454]
#   POI-half-ALL +RR3  n= 325  mean_r=+3.50  median=+0.72  ci=[0.270, 0.448]  <- shipped
#   POI-third-ALL+RR2  n= 417  mean_r=+2.52  median=+0.38  ci=[0.236, 0.393]
#   POI-half-ALL +RR2  n= 455  mean_r=+2.51  median=+0.39  ci=[0.233, 0.380]
#   spec combined      n= 508  mean_r=+2.18  median=+0.14  ci=[0.213, 0.334]
#   baseline           n=1424  mean_r=+0.79  median=-0.10  ci=[0.129, 0.196]
# The shipped row's CI doesn't overlap the baseline's at that sample size.
# See app.smc.phase's note on why 0.5/3.0 (both doctrine, neither fitted)
# was preferred over the marginally higher-scoring 0.33.
...
def _sweep_one_ticker(session, ticker: str) -> dict[str, list]:
    """One ticker through every variant -- the unit parallel_tickers farms
    out to a worker process. Returns variant name -> [(event_ts, r), ...];
    the parent decides which window each row lands in."""
    ...
    rows: dict[str, list] = {name: [] for name in VARIANTS}
    originals = (
    ...
    # on the truncated pre-event window for EVERY qualifying event -- by far
    # the most expensive step, and its result is identical across all
    # variants here (none of them touch SMCConfig or detection). Memoize it
    # keyed by truncated length, exactly as optimize_wyckoff.py does.
...
def main() -> None:
    ...
    print(f"{len(tickers)} ticker(s), {len(VARIANTS)} variant(s), {workers} process(es)\n")
