...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        print(f"{len(tickers)} VN30 ticker(s) with enough history\n")
        ...
        for i, ticker in enumerate(tickers, 1):
            ...
            # Same reasoning as optimize_wyckoff.py's cache: the phase-before-
            # event gate doesn't depend on MAX_RANGE_HEIGHT_PCT either, so
            # memoize it across the 7 variants below.
            ...
            try:
                for range_height_pct in MAX_RANGE_HEIGHT_PCT_GRID:
                    ...
                    bucket = results[range_height_pct]
